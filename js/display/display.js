...
class Display extends EventDispatcher {
    ...
    async _findDictionaryEntries(isKanji, source, wildcardsEnabled, optionsContext) {
        if (isKanji) {
            ...
            // Custom edits =================
            // This searches words, and their alterations in cases (lower, upper, etc.) so a search like Он and он will both be correctly searched.
            ...
            const searches = [firstCharLower(source), firstCharUpper(source), source.toLowerCase(), source];

            for (const search of searches) {
                const result = await yomichan.api.termsFind(search, findDetails, optionsContext);

                if (result.dictionaryEntries.length > 0) {
                    result.dictionaryEntries.forEach(entry => {
                        const {headwords} = entry;
    
                        for (const headword of headwords) {
                            const {term} = headword;
        
                            if (!matchedHeadWords.includes(term)) {
                                matchedHeadWords.push(term);
    ...
    async _setContentTermsOrKanji(type, urlSearchParams, token) {
        ...
        // Move the cursor out of the entry re-hides any open inflection boxes
        ...
        Array.from(document.querySelectorAll('.entry')).forEach(ent => {
            ...
            Array.from(document.querySelectorAll('.current-dictionary-entry .inflection')).forEach(inflect => {
            ...
            Object.entries(formBoxes).forEach(box => {
                ...
                Array.from(document.querySelectorAll('.current-dictionary-entry .definition-tag-list')).forEach(tag => {
                ...
                if (matchingTag) {
                    ...
                    if (isAutomated) formInfoBox += `<div class="automated-result-text">(automated results)</div>`;
                    ...
                    const recentInfoBtn = 
                        Array.from(document.querySelectorAll('.current-dictionary-entry .show-info-btn')).pop();
    
                    const recentInfoBox = 
                        Array.from(document.querySelectorAll('.current-dictionary-entry .form-info-box')).pop();
    
                    recentInfoBtn.addEventListener('mouseenter', (e) => {
                        recentInfoBox.style.display = 'block';
                    });
    
                    document.querySelector('.current-dictionary-entry').addEventListener('mouseleave', (e) => {
            ...
            // Custom edits =================
            // Automatically add accent class to stress in Russian word
            // Array.from(document.querySelectorAll('.current-dictionary-entry .headword-term ruby')).reverse().forEach(ruby => {
            //     const readingElem = ruby.childNodes[1];

            //     if (/́/.test(readingElem.textContent)) {
            //         let [accent, ipa] = readingElem.textContent.split(/(?=\s[\[\/])/).map(text => text.trim());

            //         let accentHTML = '';

            //         accent.split('').forEach(char => {
            //             // accent
            //             if (char === "́") {
            //                 const lastChar = accentHTML[accentHTML.length - 1];
            //                 accentHTML = accentHTML.slice(0, -1);
            //                 accentHTML += `<span class="accent">${lastChar}</span>`;
            //             } else accentHTML += char;
            //         });

            //         ruby.innerHTML = `${accentHTML}<rt>${ipa}</rt>`;
            //     }
            // })
...
// Custom edits =================
// Two functions that return text with only the first character set to lower or to upper
